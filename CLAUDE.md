...
**Description**: A comprehensive test project with all template features enabled
**Author**: Byron Williams <byron@byronwilliamscpa.com>
...
- See `src/template_sample/utils/financial.py` for utilities
- Example:
  ```python
...
docker build -t template_sample .  # Build production image
```
...
- Logging: Structured logging via `src/template_sample/utils/logging.py`
- Error Handling: Custom exceptions in `src/template_sample/core/exceptions.py`
...
class Settings(BaseSettings):
    project_name: str = "Template Sample"
...
../template_sample-worktrees/

# Quick reference commands
git worktree add ../template_sample-worktrees/feature-name -b feature/feature-name
git worktree add ../template_sample-worktrees/pr-42 origin/feature/pr-branch
git worktree list
git worktree remove ../template_sample-worktrees/feature-name
```
...
```
Closes TMPL-123    # Closes issue when PR merges
Fixes TMPL-456     # Same as closes
Refs TMPL-789      # References without closing
```
